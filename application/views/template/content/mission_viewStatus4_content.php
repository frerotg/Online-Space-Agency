<p>Votre mission est actuellement en attente d'ordre</p>
...
foreach($actions AS $action){
	if($action->id_space_object_status <= $space_object->id_space_object_status){
		echo anchor('c_mission/doAction/'.$info->id_mission.'/'.$action->id_space_action,$action->name_space_action, array('title' => 'Lancer one mission d\'/de '.$action->name_space_action, 'class' => ''));
	}
	else{
		echo $action->name_space_action;
	}
}
